...
* ``influxdb_httpd_points_written_ok``, the number of points successfully written.
...
* ``influxdb_write_point_requests``, the number of write points requests across multiple data nodes.
* ``influxdb_write_point_request_local``, the number of write points requests from local data nodes.
* ``influxdb_write_point_request_remote``, the number of write points requests across remote data node.
* ``influxdb_write_requests``, the number of write requests across multiple data nodes.
...
* ``influxdb_memory_system``, the number of bytes obtained from system.
...
* ``influxdb_heap_system``, the number of bytes obtained from system.
