...
* ``openstack_nova_service``, only present when a service is 'down' on a
  particular node and always equals to 1. The metric contains a ``service``, a
  ``state`` (always 'down') and a ``hostname`` field.
...
* ``openstack_cinder_service``, only present when a service is 'down' on a
  particular node and always equals to 1. The metric contains a ``service``, a
  ``state`` (always 'down') and a ``hostname`` field.
...
* ``openstack_neutron_agent``, only present when an agent is 'down' on a
  particular node and always equals to 1. The metric contains a ``service``, a
  ``state`` (always 'down') and a ``hostname`` field.
