...
|   List of field name / value pairs, also known as dimensions, used to select
    ...
    file system mount point. If the value is not provided,
    then the rule is applied to all the aggregated values for the specified
    field name.
    In the example above, the rule is applied to the metrics matching
    having a **fs** dimension matching the pattern **"=~ ceph/%d+$"**.
...
The alarming framework allows to specify alarms against metrics with a fine
grained filtering mechanism called **dimension pattern matching** (
`read the full specification <https://blueprints.launchpad.net/lma-toolchain/+spec/afd-alarm-fields-matching>`_
).
...
* Relational operators
  ...
  * strings only (see `Lua pattern matching <http://www.lua.org/manual/5.1/manual.html#5.4.1>`_
    for syntax)
...
Few practical examples are better than thousand of words :
