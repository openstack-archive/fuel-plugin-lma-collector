...
* ``haproxy_backend_server``, the status of the server where values ``0`` and
  ``1`` represent, respectively, ``DOWN`` and ``UP``.
  This metric has a correspondant ``state`` field (either 'down' or 'up').
