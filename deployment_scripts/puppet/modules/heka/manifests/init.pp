...
define heka (
  ...
  if $dashboard_address {
    ...
    $_dashboard_address = $heka::params::dashboard_address
  }
  if $dashboard_port {
    $_dashboard_port = $dashboard_port
  } else {
    $_dashboard_port = $heka::params::dashboard_port
  }
  ...
  if $poolsize {
    ...
    $_poolsize = $heka::params::poolsize
  }
  validate_integer($_poolsize)
  ...
  if $_dashboard_address {
    file { "${_config_dir}/dashboard.toml":
      ensure  => present,
