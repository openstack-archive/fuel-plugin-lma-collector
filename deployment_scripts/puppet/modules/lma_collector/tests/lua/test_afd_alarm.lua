...
function TestLMAAlarm:test_no_datapoint()
    ...
    for _, result in ipairs(results) do
        assertEquals(result.alert.message, 'No datapoint has been received ever')
...
function TestLMAAlarm:test_rules_logical_missing_datapoint__op_and()
    ...
    assertStrContains(result[1].alert.message, 'No datapoint has been received over the last')
    ...
    assertStrContains(result[2].alert.message, 'No datapoint has been received over the last')
