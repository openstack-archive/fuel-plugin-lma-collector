...
function TestLMAAlarm:test_no_datapoint()
    ...
    for _, result in ipairs(results) do
        assertEquals(result.alert.message, 'No datapoint for evaluation')
...
function TestLMAAlarm:test_rules_logical_missing_datapoint__op_and()
    lma_alarm.load_alarm(alarms[6])
    lma_alarm.set_start_time(next_time())
    ...
    assertEquals(state, consts.OKAY)
    assertEquals(#result, 0)
    local state, result = lma_alarm.evaluate(next_time(200)) -- 100s w/o datapoint
    assertEquals(state, consts.OKAY)
    local state, result = lma_alarm.evaluate(next_time(41)) -- 241s w/o datapoint
    assertEquals(state, consts.UNKW)
    assertEquals(result[1].alert.metric, 'cpu_idle')
    assertStrContains(result[1].alert.message, 'Missing datapoints for the last')
    assertEquals(result[2].alert.metric, 'cpu_wait')
    assertStrContains(result[2].alert.message, 'No datapoint')
    -- datapoints come back
