...
module Puppet::Parser::Functions
  newfunction(:get_cluster_names, :type => :rvalue) do |args|
    ...
    roles = args[1]
    raise Puppet::ParseError, "roles passed to get_cluster_names is not a list" unless data.is_a?(Array)
    ...
    roles.each do |role|
      ...
      # name that matches with role. So add "_default" name.
      cluster_names["node"].add("_default") if cluster_names["node"].empty?
