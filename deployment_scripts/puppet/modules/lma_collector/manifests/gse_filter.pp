...
#    under the License.
define lma_collector::gse_filter (
  $input_message_type,
  $entity_field,
  $metric_name,
  $interval = 10,
  ...
  heka::filter::sandbox { "gse_${title}":
    ...
    filename        => "${lma_collector::params::plugins_dir}/filters/gse_filter.lua",
    message_matcher => "Type == '${input_message_type}' or Type == 'heka.sandbox.${input_message_type}'",
    ticker_interval => 1,
    config          => {
