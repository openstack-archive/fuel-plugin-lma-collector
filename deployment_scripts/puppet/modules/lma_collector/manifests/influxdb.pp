...
class lma_collector::influxdb (
  ...
  heka::encoder::sandbox { 'influxdb':
    ...
    config     => {
      ...
      # FIXME(pasquier-s): provide the tenant_id & user_id parameters but this
      # requires to request Keystone since we only have access to the tenant
      # name and user name for services
      default_tenant_id => 'to_be_fixed',
      default_user_id   => 'to_be_fixed',
    },
  ...
  heka::output::http { 'influxdb':
    config_dir      => $lma_collector::params::config_dir,
    url             => "http://${server}:${port}/write/?db=${database}",
