        ...
        for node in config.children:
            if node.key == "Url":
                ...
                if len(node.values) > 2:
                    self.urls_must_return[node.values[0]] = int(node.values[2])
                else:
                    self.urls_must_return[node.values[0]] = 200
    ...
    def itermetrics(self):
        for name, url in self.urls.items():
            try:
                ...
                if r.status_code != self.urls_must_return[name]:
                    ...
                    data = r.text
                    self.logger.debug(
                        "Got response from %s: '%s'" % (name, data))
