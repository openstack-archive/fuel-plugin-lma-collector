...
class RabbitMqPlugin(base.Base):
    ...
    def itermetrics(self):
        ...
        stats['pmap_used'] = 0
        stats['pmap_shared'] = 0
        stats['vm_memory_high_watermark'] = 0.0
        ...
        for v in ('vm_memory_high_watermark', 'vm_memory_limit',
                  'disk_free_limit', 'disk_free'):
            try:
                stats[v] = float(re.findall('%s,([0-9]+.[0-9]+)' % v, out)[0])
            except:
        ...
        try:
            stats['memory_total'] = int(re.findall('total,([0-9]+)',
                                                   mem_str[0])[0])
        except:
