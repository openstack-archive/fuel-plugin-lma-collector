...
class NovaStatsPlugin(openstack.CollectdPlugin):
    ...
    def read_callback(self):
        ...
        for worker in self.workers_state('nova'):
            if isinstance(worker, dict):
                host = worker['host'].split('.')[0]
                service = nova_re.sub('', worker['service'])
                state = worker['state']

                if service not in data:
                    data[service] = {'up': 0, 'down': 0, 'disabled': 0}

                if state not in data[service]:
                    self.logger.warning(
                        "Wrong nova worker state {}".format(state))
                    continue

                data[service][state] += 1
                self.dispatch_value('nova_service', '', states[state],
                                    {'host': host,
                                     'service': service,
                                     'state': state})
        ...
        for key, val in data.iteritems():
            for state, count in val.iteritems():
                self.dispatch_value('nova_services', '', count,
