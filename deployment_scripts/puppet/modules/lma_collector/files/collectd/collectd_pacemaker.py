...
class CrmMonitorPlugin(base.Base):
    ...
    def itermetrics(self):
        ...
        if self.notify_resource:
            # Notify to the other plugins whether the resource runs locally or
            # not
        ...
        for resource_id, resource_name in self.resources.iteritems():
            ...
            for item in resource_elts:
                ...
                if item.tag == 'resource':
                    # Report whether or not the resource runs locally for
                    # simple resources
                    yield {
                        'type_instance': 'resource_local_active',
                        'values': same_hostname(item.find('node')),
