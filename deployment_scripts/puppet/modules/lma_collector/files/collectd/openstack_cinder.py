...
class CinderStatsPlugin(openstack.CollectdPlugin):
    ...
    def read_callback(self):
        ...
        os_services_r = self.get('cinder', 'os-services')
        ...
        if r_status == 200 and 'services' in r_json:
            for val in r_json['services']:
                service = cinder_re.sub('', val['binary'])
                ...
                self.dispatch_value('cinder_service', service, 1, meta)

            for key, val in data.iteritems():
                for state, count in val.iteritems():
                    meta = {'state': state}
                    self.dispatch_value('cinder_services', key, count, meta)
        else:
            self.logger.warning("cinder state:{}:{}".format(r_status, r_json))
