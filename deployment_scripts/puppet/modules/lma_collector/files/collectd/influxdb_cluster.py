...
METRICS_BY_NAME = {
    'cluster': {
        'writeShardPointsReq': ('write_shard_points_requests', 'gauge'),
        'writeShardReq':       ('write_shard_requests', 'gauge')},

    'httpd': {
        'authFail':        ('httpd_failed_auths', 'gauge'),
        'pingReq':         ('httpd_ping_requests', 'gauge'),
        'pointsWrittenOK': ('httpd_points_written_ok', 'gauge'),
    ...
    'write' : {
        ...
        'pointReqLocal':  ('write_point_request_local', 'gauge'),
        'pointReqRemote': ('write_point_request_remote', 'gauge'),
    ...
    'runtime' : {
        ...
        'PauseTotalNs':   ('gc_pause_total', 'gauge'),
...
class InfluxDBClusterPlugin(base.Base):
    ...
    def itermetrics(self):
        ...
        url = "http://{}:{}/query".format(self.url, self.port)
