...
class MemcachedCheckPlugin(base.Base):
    ...
    def read_callback(self):
        try:
            ...
            now = datetime.now()
            mc.set('stacklight_check', str(now))
            val = mc.get('stacklight_check')
            msg = None
            if val != str(now):
                msg = 'Inconsitent value retrieved '
                'from memcached ({}:{}): {} vs {}'.format(
                    self.host, self.port, now, val)
        ...
        self.dispatch_check_metric(status, msg)
