...
local pid  = patt.Pid
local severity = l.Cg(l.P"debug" + l.P"info" + l.P"warning" + l.P"error", "Severity")
...
function process_message ()
    ...
    -- Logs in libvirt are not compliant with rfc3339. The extension "+0000"
    -- is hardcoded in the timestamp so we replace it with its compliant
    -- version.
    local m = grammar:match(log:gsub("+0000:", "+00:00:"))
    if not m then
        return -1
    end
