...
function process_message ()
    ...
    for _, sample in ipairs(samples) do
        ...
        local metric_without_hostname = {
            -- Add "metric_source = true" to skip the hostname for the metric
            openstack_nova_total_free_disk = true,
            openstack_nova_total_free_ram = true,
            openstack_nova_total_free_vcpus = true,
            openstack_nova_total_running_instances = true,
            openstack_nova_total_running_tasks = true,
            openstack_nova_total_used_disk = true,
            openstack_nova_total_used_ram = true,
            openstack_nova_total_used_vcpus = true,
            check_openstack_api = true,
        }
        local add_hostname = not metric_without_hostname[metric_source]
        ...
        for i, value in ipairs(sample['values']) do
            ...
            if add_hostname then
                ...
                msg['Fields']['tag_fields'] = { 'hostname' }
            end
            ...
            elseif metric_source ==  'libvirt' then
                ...
                -- Hostname is read from config because in that case, collectd
                -- sends the instance's ID instead of the hostname in the 'host'
                -- attribute.
