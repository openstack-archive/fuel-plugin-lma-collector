...
function process_message ()
    ...
    -- Logger is of form "nova-api"
    local logger = read_message("Logger")
    local service = string.gsub(logger, '(%w+)-.*', '%1')

    local log = read_message("Payload")

    -- Ignore logs that don't contain the service name
    if not string.match(log, service) then
        return 0
    end

    -- Get the program name, in the form "nova-api", dealing with the case where we
    -- get "nova-nova-api", which happens when the log file name starts with
    -- the service name
    local programname = string.format('%s-%s', service, string.gsub(logger, service .. '%-', ''))

    local m

    m = openstack_grammar:match(log)
    if not m then
        return -1, string.format("Failed to parse %s log: %s", programname, string.sub(log, 1, 64))
    end

    -- Change Logger to form "openstack.nova"
