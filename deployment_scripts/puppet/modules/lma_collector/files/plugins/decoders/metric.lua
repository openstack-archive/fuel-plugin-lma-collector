...
local loggers = loggers_pattern:match(read_config('deserialize_bulk_metric_for_loggers') or '')

local function logger_matches(logger)
    if loggers ~= nil and logger == loggers[logger] then
        return true
    end
    return false
end
...
function process_message ()
    ...
    if string.match(msg.Type, 'bulk_metric$') and logger_matches(msg.Logger) then
