...
function process_message()
    ...
    if not haproxy_backend_states[service] then
        haproxy_backend_states[service] = {up=0, down=0}
    end
    ...
    if metric_name == 'haproxy_backend_servers' then
        ...
        haproxy_backend_states[service][state] = value
    else
    ...
    if not (haproxy_backend_states[service].up and not haproxy_backend_states[service].down) then
        if not check_api_states[service] then
