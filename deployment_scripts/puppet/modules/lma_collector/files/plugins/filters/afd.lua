...
-- hostname for node AFD / service for service AFD
local afd_entity = read_config('afd_entity') or error('afd_entity must be specified!')
...
function timer_event(ns)
  if A.is_started() then
    ...
    for _, alarm in ipairs(alarms) do
        afd.add_to_alarms(
          alarm.state,
          alarm.alert.rule.fct,
          alarm.alert.rule.metric_name,
          alarm.alert.fields,
          {},
          alarm.alert.rule.relational_operator,
          alarm.alert.value,
          alarm.alert.rule.threshold,
          alarm.alert.rule.window,
          alarm.alert.rule.periods,
          alarm.alert.message)
    end
