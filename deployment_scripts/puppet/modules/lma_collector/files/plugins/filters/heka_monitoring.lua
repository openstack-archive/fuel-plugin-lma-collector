...
function extract_data_from_json(filter_table)
    -- We extract:
    --   - the name of the metric
    --   - the number of processed messages
    --   - the average duration for processing a message (nanoseconds)
    -- We return information as a tuple

    fname = filter_table['Name']
    msgCount = filter_table['ProcessMessageCount']
    avgDuration = filter_table['ProcessMessageAvgDuration']

    return fname, msgCount['value'], avgDuration['value']
end

function process_table(datapoints, timestamp, hostname, array)
    ...
    for _, v in pairs(array) do
        if type(v) == "table" then
            ...
            utils.add_metric(datapoints,
                             string.format('%s.%s.msg_count', hostname, name),
                             {timestamp, msgCount})
            utils.add_metric(datapoints,
                             string.format('%s.%s.avg_duration', hostname, name),
...
function process_message ()
    ...
    for k, v in pairs(json) do
        if k == "filters" or k == "decoders" then
            process_table(datapoints, ts_ms, hostname, v)
        end
    end
