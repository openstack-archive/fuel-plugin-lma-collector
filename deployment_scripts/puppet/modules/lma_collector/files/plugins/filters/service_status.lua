...
function process_message ()
    ...
    for service_name, service in pairs(services) do
        ...
        if not expired(ts, data.vip_active_at) then
            local event_title
            local prev = service_status[service_name].general_status or utils.general_service_status_map.UNKNOWN
            if prev and prev ~= general_status then
                event_title = string.format("General status %s -> %s",
                                      utils.general_service_status_to_label_map[prev],
                                      utils.general_service_status_to_label_map[general_status])
