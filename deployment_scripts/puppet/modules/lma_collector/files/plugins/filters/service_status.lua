...
--    if elts_name == 'api' then debug(service[elts_name]) return 1 end
    ...
    if total_elts == 0 or status == nil then status = utils.service_status_map.UNKNOWN end
    ...
    --debug({service=name, zero_up=zero_up, down_elts=down_elts, one_up=one_up, total=total_elts})
    return status
end
...
function timer_event(ns)
    ...
    for service_name, service in pairs(services) do
        ...
        status_api = compute_status(datapoints, current_time, 'api', service_name, service)
