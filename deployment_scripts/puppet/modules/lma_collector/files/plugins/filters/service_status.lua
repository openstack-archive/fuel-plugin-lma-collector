...
function process_message ()
    ...
    for service_name, service in pairs(services) do
        ...
        if service.workers then
            worker_status = compute_status(events, ts, 'workers', service_name, service.workers)
        end
        ...
        haproxy_server_status = compute_status(events, ts, 'haproxy', service_name, service.haproxy)
        ...
        if not expired(ts, data.vip_active_at) then
            ...
            elseif #events > 0 then
                event_title = string.format("General status stays %s",
                                      utils.service_status_to_label_map[general_status])
            end
            if event_title then
