...
function process_message ()
    ...
    for service_name, service in pairs(services) do
        ...
        if service.workers then
            worker_status = compute_status(events, other_status, ts, 'workers', service_name, service.workers)
        end
        ...
        haproxy_server_status = compute_status(events, other_status, ts, 'haproxy', service_name, service.haproxy)
        ...
        if not expired(ts, data.vip_active_at) then
            ...
            if event_title then
                 for k, v in pairs(other_status) do events[#events+1] = v end
...
function compute_status(events, other_status, current_time, elts_name, name, service)
    local down_elts = {}
    ...
    for worker_name, worker in pairs(zero_up) do
        ...
        if prev and prev ~= DOWN then
            ...
            other_status[#other_status+1] = string.format("%s status %s", worker_name,
                                              utils.service_status_to_label_map[DOWN])
        end
