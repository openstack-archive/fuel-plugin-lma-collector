...
local bulk_size = (read_config('bulk_size') or 500) + 0
local percentile_thresh = (read_config('percentile') or 90) + 0

local inject_reached_error = 'too many metrics to aggregate, consider adjusting bulk_size and/or max_timer_inject parameters'

local msg_source = 'http_metric_filter'
local last_ticker = os.time() * 1e9
...
function process_message ()
    ...
    if timestamp < last_ticker - (interval_in_ns / 2) then
    ...
    if not all_times[service][http_method][http_family_status] then
        ...
        all_times[service][http_method][http_family_status] = tab.deepcopy(metric_bucket)
        num_metrics = num_metrics + 1
    end

    local bucket = all_times[service][http_method][http_family_status]
            ...
            for status, bucket in pairs(statuses) do
                ...
                if bucket.count > 1 then
                    ...
                    if idx > 0 and bucket.times[idx] then
                        bucket.percentile = bucket.times[idx]
                    end
                end
                bucket.times = nil
                utils.add_to_bulk_multivalue_metric(metric_name, bucket, {http_method=method, http_status=status})
