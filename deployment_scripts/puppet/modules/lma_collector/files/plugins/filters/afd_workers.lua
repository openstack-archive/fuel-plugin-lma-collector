...
function process_message()
    ...
    elseif worker.down >= worker.up then
        ...
        afd.add_to_alarms(consts.CRIT,
                          ...
                          nil,
                          nil,
                          string.format("The number of %s instances which are down is greater than or equal to the number of up instances", service))
    elseif worker.down > 0 then
        ...
        afd.add_to_alarms(consts.WARN,
                          ...
                          nil,
                          nil,
                          string.format("At least one %s instance is down", work.down, service, worker.up))
    end

    afd.inject_afd_service_event(service,
                                 state,
