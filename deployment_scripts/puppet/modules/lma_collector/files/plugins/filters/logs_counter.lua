...
function process_message ()
    ...
    if not logs_counters[service] then
        ...
        logs_counters[service]['last_sending'] = 0
...
function timer_event(ns)
    ...
    if ns - enter_at < interval_in_ns and current_service <= #discovered_services then
        ...
        local last_sending = logs_counters[service_name]['last_sending']
        local delta_sec = (ns - last_sending) / 1e9

        for level, val in pairs(logs_counters[service_name]) do

           -- We don't send the first value
           if last_sending ~= 0 and delta_sec ~= 0 then
               msg.Fields = {
                   name = 'log_messages',
        ...
        logs_counters[service_name]['last_sending'] = ns
