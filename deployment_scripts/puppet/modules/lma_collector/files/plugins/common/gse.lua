...
-- facts about the cluster's members
local facts = {}
local clusters = {}
...
function set_status(cluster_id, member, value, alarms, hostname)
    ...
    if not facts[member] then
        facts[member] = {}
    end
    facts[member][group_key] = {
        status=value,
        alarms=alarms,
        hostname=hostname
    }
...
function resolve_status(cluster_id)
    ...
    local cluster_status = consts.UNKW
    local alarms = {}

    for _, member in ipairs(cluster.members) do
        for _, fact in pairs(facts[member] or {}) do
            ...
            cluster_status = max_status(cluster_status, status)
        end
    end
    cluster.fact = {
        status=cluster_status,
        alarms=lma.deepcopy(alarms)
    }

    if cluster_status ~= consts.OKAY then
        -- add hints if the cluster isn't healthy
        for _, member in ipairs(cluster.hints or {}) do
            local other_cluster = clusters[member]
            if other_cluster and other_cluster.fact then
                local status = other_cluster.fact.status
                if status ~= consts.OKAY then
                    for _, v in ipairs(other_cluster.fact.alarms) do
                        alarms[#alarms+1] = lma.deepcopy(v)
                        if not alarms[#alarms]['tags'] then
                            alarms[#alarms]['tags'] = {}
                        end
