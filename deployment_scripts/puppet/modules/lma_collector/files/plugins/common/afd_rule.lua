...
function Rule.new(rule, datastore)
  ...
  if rule.window then
    rule.window = rule.window + 0
    if rule.window > 0 then
      win = rule.window
    end
  end
  r.window = win
  local periods = MIN_PERIOD
  if rule.periods then
    rule.periods = rule.periods + 0
    if rule.periods > 0 then
...
function Rule:fields_accepted(metric, fields)
    if not fields then
        fields = {}
    end
    --pring(' ')
    --pring('Compare fields:')
    --pring (inspect(fields))
    --pring (inspect(self.fields))
    local acceptations = {}
    for f, wanted in pairs(self.fields) do
        for k, v in pairs(fields) do
            --pring(' .. ' .. k .. '==' .. f .. ' ' .. v .. '==' .. wanted)
            if k == f and wanted == '*' then
                acceptations[#acceptations+1] = true
            elseif k == f and v == wanted then
                acceptations[#acceptations+1] = true
            elseif k == f and v ~= wanted then
                acceptations[#acceptations+1] = false
            end
        end
    end
    --pring(inspect(acceptations))
    for _, w in ipairs(acceptations) do
        if w == false then
            --pring('FALSE')
            return false
        end
...
function Rule:add_value(ts, metric, value, fields)
  if not self:fields_accepted(metric, fields) then
      return
  end
  local data
