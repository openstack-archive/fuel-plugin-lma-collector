...
function Alarm:evaluate(ns)
    ...
    for _, rule in ipairs(self.rules) do
        ...
        elseif eval == afd.MISSING_DATA then
           local msg = 'No datapoint has been received over the last ' .. rule.observation_window .. ' seconds'
           add_alarm(rule, -2, msg, convert_field_list(rule.fields))
           one_unknown = true
        elseif eval == afd.NO_DATA then
           add_alarm(rule, -1, 'No datapoint has been received ever', convert_field_list(rule.fields))
           one_unknown = true
        end
    end
