...
function diag_collectd {
  ...
  cp -rf /usr/lib/collectd/*py "${diag_output}/plugins/"
  tail -n 10000 /var/log/collectd.log > "${diag_output}/last.log"
...
function diag_influxdb {
  ...
  tail -n 10000 /var/log/influxdb/influxd.log > "${diag_output}/last.log"
  ...
  tail -n 10000 /var/log/grafana/grafana.log> "${diag_output}/last.log"
...
function diag_elasticsearch {
  ...
  tail -n 10000 /var/log/elasticsearch/es-01/*.log> "${diag_output}/last.log"
  # Get previous logs
  for l in $(ls /var/log/elasticsearch/es-01/*.log.2*| tail -n 2); do
    tail -n 10000 "$l" >> "${diag_output}/previous.log"
  done
...
function diag_collector {
  ...
  for l in $log_file; do
    ...
    tail -n 10000 "$l" > "$logs_dir/${collector_name}"
  done
}
...
function diag_nagios {
  ...
  tail -n 1000 /var/nagios/nagios.log > $diag_output/last.log
  tail -n 1000 /var/log/apache2/nagios_error.log $diag_output/apache_error.log
  tail -n 1000 /var/log/apache2/nagios_access.log $diag_output/apache_access.log
