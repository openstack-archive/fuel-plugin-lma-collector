...
function tail_file {
    ...
    mkdir -p $(dirname "$out")
    num=${3:-10000}
    test -f "$file" && tail -n $num "$file" >> "$out" 2>&1
    return $?
}
...
function diag_collectd {
  ...
  tail_file /var/log/collectd.log "$diag_output"
...
function diag_influxdb {
  ...
  tail_file /var/log/influxdb/influxd.log "$diag_output"
  ...
  tail_file /var/log/grafana/grafana.log "$diag_output"
...
function diag_nagios {
  ...
  tail_file /var/nagios/nagios.log "$diag_output"
  tail_file /var/log/apache2/nagios_error.log "$diag_output"
  tail_file /var/log/apache2/nagios_access.log "$diag_output"
