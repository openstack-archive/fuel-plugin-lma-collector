...
case $influxdb_mode {
  'remote','local': {
    ...
    if $is_base_os {
      $influxdb_present = $influxdb_node_name == $influxdb_grafana['node_name'] and $influxdb_mode == 'local'
      $elasticsearch_present = $es_node_name == $elasticsearch_kibana['node_name'] and $elasticsearch_mode == 'local'

      if $influxdb_present and $elasticsearch_present {
        # Elasticsearch is running on a JVM
        $processes       = ['hekad', 'collectd', 'influxdb']
        $process_matches = [{name => 'elasticsearch', regex => 'java'}]
      } elsif $influxdb_present {
        $processes = ['hekad', 'collectd', 'influxdb']
        $process_matches = undef
      } elsif $elasticsearch_present {
