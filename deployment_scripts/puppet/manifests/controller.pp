...
if $influxdb_mode != 'disabled' {
  ...
  if $use_influxdb {
    $influxdb_url = "http://${influxdb_server}:${influxdb_port}"
    $grafana_url = "http://${influxdb_server}:${grafana_port}"
  }
  ...
  $vip_urls = {
    'vip_nagios'        => $nagios_url,
    'vip_kibana'        => $kibana_url,
    'vip_elasticsearch' => $elasticsearch_url,
    'vip_influxdb'      => $influxdb_url,
    'vip_grafana'       => $grafana_url,
  }
  $expected_codes = {
    'vip_nagios'        => 401,
    'vip_influxdb'      => 204,
  ...
  class { 'lma_collector::collectd::check_http':
